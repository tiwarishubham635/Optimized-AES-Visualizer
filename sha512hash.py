...
def ascii_to_string(bin_string):
    ans = ""
    for i in range(0, len(bin_string), 8):
        bin_str = "0b" + bin_string[i:i+8]
        number = int(bin_str, base=2)
        ascii_val = chr(number)
        ans += ascii_val
    return ans
...
def XOR(a, b): return ''.join(str(int(i) ^ int(j)) for i, j in zip(a, b))
def AND(a, b): return ''.join(str(int(i) & int(j)) for i, j in zip(a, b))
def OR(a, b): return ''.join(str(int(i) | int(j)) for i, j in zip(a, b))
def NOT(a): return ''.join(str((not int(i)) & 1) for i in a)
...
def hash_it(code):
    # code="kasra"
    def padding(num, m_len): return "0"*(m_len-len(num))+num
    ...
    # print(H)
    # n = int('0b'+H[0], 2)
    # TEMP = binascii.unhexlify('%x' % n)
    # print(TEMP)
    # ans = ""
    # ans = ans + str(TEMP)
    # print(len(H[0]))
    ans = copy.deepcopy(ascii_to_string(H[0]) + ascii_to_string(H[1]))

    # code = "".join(H)
    # code = hex(int(code, base=2))
    #res_str = ascii_to_string(hashcode)

    return ans
    # print(res_str)


# print(ascii_to_string("0100000101000010"))
# print(hash_it("hello"))
# print(hash_it("hello"))
